...
function addLicensePlate(cars) {
    for (let i = 0; i < cars.length; i++) {
        cars[i].licensePlate = "EV077BF";
    }
    return cars;
}

let licensePlate = cars.map(car => ({...car, licensePlate: "EV077BF"}))
...
let newCar = function() {
    cars.push({
        ...
        model: "Panda4x4",
        color: "white",
        trims: ["4x4", "bestemmie", "immortalità"],
...
let arreyNuovo = cars.filter(car => car.trims.length > 2);
console.log(arreyNuovo);
...
let arreyNuovo2 = cars.map(car => car.brand + " " + car.model);
console.log(arreyNuovo2);
